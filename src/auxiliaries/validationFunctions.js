...
// mínimo 8 caracteres, 1 minúscula, 1 mayúscula, 1 número, 1 caracter especial
export const isValidPassword = (pwd) => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/.test(pwd);
...
export const pwdRules = (val, oldPwd) => {
  const rules = [
    { key: 'length',  label: 'Al menos 8 caracteres',        test: () => val.length >= 8 },
